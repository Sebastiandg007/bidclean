...
| `PropertyDetailScreen.tsx` | **Implemented.** Full property detail: inline photo gallery (horizontal ScrollView with tap-to-fullscreen Modal, FlatList swipe navigation, photo counter), PropertyMap in view-only mode, info cards grid (area, bedrooms, bathrooms, floor, parking, elevator), checklist section (read-only list), requirements chips (read-only styled chips with i18n labels), access instructions card (private), offer-readiness indicator badge (green/warning), Edit and Publish Offer action buttons (placeholder alerts). Loading state with ActivityIndicator, error state with retry. All text via i18n, full accessibility (roles, labels, testID). |
| `CreatePropertyScreen.tsx` | **Implemented.** Multi-step form (3 steps) with useReducer state management: Step 1 (name, PropertyTypeSelector, description, sqm, bedrooms, bathrooms) → Step 2 (AddressInput, geocode button, PropertyMap with manual pin fallback, reverse geocoding on pin move) → Step 3 (PhotoUploader placeholder with note, ChecklistEditor, RequirementsChips, access instructions, floor, parking/elevator toggles). Per-step validation, animated step indicator, saves via store's createProperty() with Idempotency-Key. |
| `EditPropertyScreen.tsx` | **Implemented.** Pre-populated edit form using property detail data. Same multi-step structure as Create (3 steps: Basic Info → Address + Map → Details). Fetches property via `fetchDetail(propertyId)`, initializes form from existing data, validates per step, saves only changed fields via PATCH (`updateProperty`). Sets `locationSource = 'GEOCODED'` on address re-geocode, `locationSource = 'MANUAL'` on pin move. Photos displayed read-only (managed from PropertyDetailScreen). Loading state, error with retry, animated step transitions. |
...
| `components/PropertyPhotoGallery.tsx` | **Implemented.** Reusable self-contained horizontal photo gallery: ScrollView thumbnails with snap behavior, tap-to-fullscreen Modal (FlatList + pagingEnabled for swipe navigation), photo counter in both inline ("3 photos") and full-screen ("2 / 5") views, empty state, full accessibility (roles, labels, testID). Manages its own modal state internally — usable anywhere without external state. |
...
| `components/RequirementsChips.tsx` | **Implemented.** Predefined requirement chips (from PREDEFINED_REQUIREMENTS constant) with solid border styling + custom text input for user-defined requirements with dashed border styling. Multi-select toggle, max count validation (PROPERTY_MAX_REQUIREMENTS), count indicator badge, empty state, limit-reached message. Full accessibility (roles, labels, states) and testID attributes. All text via i18n. |
