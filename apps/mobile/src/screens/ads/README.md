...
| `providers/admob.provider.ts` | `AdMobAdProvider` — concrete provider backed by Google AdMob (`react-native-google-mobile-ads`, loaded defensively); renders the banner through a dedicated `AdMobBannerView` wrapper that owns the native ad lifecycle 1:1 with the slot (release-on-unmount, Req 6.4), forwards only paid impressions (ILRD) with a monotonic sequence so same-millisecond impressions stay distinct, collapses the slot on no-fill/error, imports no RevenueCat and never decides eligibility |
| `useAdSlot.ts` | Pure selector of the layered render decision (`adsEnabled AND providerReady AND placementAllowed AND consentResolved`) plus provider bindings. Request-once-per-mount and release-on-unmount are realized by the render topology (one `AdBanner` → one native view per slot mount), not imperative state here. Reads eligibility only from `useAdVisibility` (never PRO/tier), asserts the Cleaner-only radar slot invariant, and wires `onPaidImpression` to `useAds.reportImpression` (never calls RevenueCat directly) |
...
| `__tests__/admob-provider.spec.tsx` | Unit/lifecycle tests for `AdMobAdProvider.renderAdView`: collapses (onNoFill) when the unit id is empty, and mounts/unmounts the native banner wrapper without throwing (release-on-unmount, Req 6.4) |
