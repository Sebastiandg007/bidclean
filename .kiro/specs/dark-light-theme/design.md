...
- **The design-token layer is the single source of truth for color.** No screen defines its own palette or inline hex. The primitive brand palette (obsidian, surface, mint, white, warm off-white, danger…) is mapped into **semantic tokens** (`background`, `surface`, `textPrimary`, `accent`, `border`, `danger`, `onAccent`, …), and each theme resolves those tokens to concrete per-mode values. Screens consume semantic tokens via `useTheme()` — never raw hex, never a primitive directly.
...
6. **Migration is mandatory.** Every existing local `const COLORS` / inline hex is migrated to `useTheme()` tokens; a hex search over application color values then finds only the token layer (external assets and third-party internals exempt).
...
1. **One token layer, semantic over primitive.** Screens consume semantic roles, not brand seeds, so a palette tweak (e.g. light-mode surface) is one edit in the token layer, and accent-as-CTA is expressible without leaking `#00F5D4` into screens (REQ-TH1, REQ-TH4).
2. **Context provider, not a Zustand store, for the *resolved theme* value.** The resolved theme must be read by nearly every component on every render; React context is the idiomatic, prop-drill-free way and gives a single re-render on mode change. Zustand (the project's state tool) still backs the **persistence + mode transitions** via a small `useThemeStore` that the provider subscribes to — so we keep the project's Zustand convention for the *stateful* part while context distributes the *derived value*. This avoids prop-drilling and guarantees app-wide consistency (REQ-TH8).
3. **Resolution gate + native splash hold for no-FOUC.** `expo-splash-screen` `preventAutoHideAsync()` is called at module load; `ThemeProvider` hides the splash (`hideAsync()`) only after `(persisted mode, OS scheme)` both resolve and the first themed frame is ready. Until then it renders nothing themed. This turns "no flash" into a testable state machine, not a hope (REQ-TH7).
...
flowchart TD
    subgraph Tokens[Token layer — single source of truth NEW]
        Prim[primitive palette\nobsidian #0B0C10, surface #1F2833,\nmint #00F5D4, white, warmOffWhite ~#F5F2EB, danger]
        ...
        Dark[darkTheme values default/reference]
        Light[lightTheme values additive parity]
        Prim --> Dark
        Prim --> Light
...
└── src/
    └── theme/                          (EXISTING folder — now actually populated & consumed)
        ├── primitives.ts               (raw brand seeds — the ONLY place raw hex lives, besides theme values)
        ├── tokens.ts                   (SemanticTokens TYPE — the token shape; ThemeMode & ResolvedTheme enums)
        ├── dark.theme.ts               (darkTheme: SemanticTokens — brand reference values, the default)
        ├── light.theme.ts              (lightTheme: SemanticTokens — warm off-white parity values)
...
sequenceDiagram
    ...
    UI->>Splash: onLayout → SplashScreen.hideAsync()
    Note over UI: app was NEVER shown in a default theme; no repaint
```
...
sequenceDiagram
    ...
    Prov->>Prov: persist { version, mode } (expo-secure-store)
    Prov->>Prov: recompute resolvedTheme(next, OS scheme)
    Prov->>Ctx: publish { theme, mode, resolvedTheme, setMode }
    Ctx-->>Screens: single consistent re-render (no prop-drilling, no partial theming)
...
```typescript
// primitives.ts — raw brand seeds; NOT consumed by screens. One of only two places hex lives.
export const palette = {
  ...
  white: '#FFFFFF',
  warmOffWhite: '#F5F2EB',
  // …near-black text, light surface, danger, success, overlay scrims per mode
} as const;

// dark.theme.ts — the DEFAULT and reference. Brand values verbatim.
export const darkTheme: SemanticTokens = {
  background: palette.obsidian,      // #0B0C10
  surface: palette.surfaceDark,      // #1F2833
  surfaceElevated: '…',
  textPrimary: palette.white,        // #FFFFFF
  textSecondary: '…', textMuted: '…',
  accent: palette.mint,              // #00F5D4 (CTA/active only)
  onAccent: palette.obsidian,        // dark text on mint
  border: '…', divider: '…', danger: '…', success: '…', overlay: '…',
};

// light.theme.ts — additive parity. Warm off-white, not pure white. Same mint accent.
export const lightTheme: SemanticTokens = {
  background: palette.warmOffWhite,  // ~#F5F2EB (warm, not #FFFFFF)
  surface: '…lighter warm surface…',
  surfaceElevated: '…',
  textPrimary: '…near-black…',
  textSecondary: '…', textMuted: '…',
  accent: palette.mint,              // #00F5D4 (still CTA/active only)
  onAccent: '…', border: '…', divider: '…', danger: '…', success: '…', overlay: '…',
};
...
> Exact light-mode values are a design decision seeded by the plan; they are chosen to satisfy the WCAG 2.1 AA criterion (Requirement 2.3) for the text/essential-UI token pairs, verified by `contrast.ts` in tests. `assertTokenParity()` runs in dev/CI so a dark-only or light-only token is caught immediately (REQ-TH2).
...
interface ThemeStoreState {
  ...
  load(): Promise<void>;    // read expo-secure-store; parse+validate; fallback DARK on any failure
  setMode(next: ThemeMode): Promise<void>; // update + persist { PREFERENCE_VERSION, next }
}
```
...
- `setMode(next)` updates in-memory `mode` then persists `{ version: PREFERENCE_VERSION, mode: next }`. A persistence failure still applies the mode for the session (the render must not be blocked); the failure is logged, not swallowed silently.
...
- `resolved` is `true` only when `store.isLoaded === true` AND the OS scheme has been read. **Before `resolved`, the provider renders `null`** (nothing themed) while the native splash is still visible — this is the no-FOUC gate (REQ-TH7). It never renders a default-theme tree first.
...
- `theme = THEMES[resolvedTheme]`. The context value is memoized so a mode change produces exactly one consistent re-render across the tree (REQ-TH8), with no prop-drilling.
...
*For all* combinations of `(isLoaded, osSchemeAvailable) ∈ { true, false }²`, the provider's `resolved` flag SHALL be `true` **if and only if** `isLoaded ∧ osSchemeAvailable`, and while `resolved` is `false` the provider SHALL produce **no themed content** (it renders `null`, the native splash stays held). The provider SHALL never expose a `resolvedTheme` computed from a default assumption before both inputs are available — i.e. there is no state in which themed content mounts under a placeholder theme and later repaints.

**Validates: Requirements 3.3, 7.4** · REQ-TH7

### Property 4: Preference load robustness (safe default, never throws)

*For all* stored payloads `b` — including a valid current-version preference, a missing/`null` value, malformed JSON, a non-object, an unknown `mode` not in `ThemeMode`, an absent or unrecognized `version`, and a storage read that throws — `load(b)` SHALL return a valid `ThemeMode`, SHALL NOT throw, and SHALL complete (never block indefinitely). The returned mode SHALL equal the stored `mode` **if and only if** `b` is a well-formed preference of a supported `version` with a valid `mode`; in every other case it SHALL equal `DEFAULT_MODE` (`DARK`).

**Validates: Requirements 3.4, 3.5** · REQ-TH5
...
No error path renders a wrong-theme frame, crashes on a bad preference, or blocks the app on the splash indefinitely.
...
| P3 Resolution gate | `(isLoaded, osSchemeAvailable) ∈ {true,false}²` | `resolved === (isLoaded && osSchemeAvailable)`; no themed output while unresolved; no default-then-repaint state exists |
| P4 Load robustness | Arbitrary stored payloads: valid current-version prefs, missing/null, malformed JSON strings, non-objects, unknown `mode`, wrong/absent `version`, a throwing store stub | `load` always returns a valid `ThemeMode`, never throws, terminates; equals stored `mode` iff well-formed+supported+valid, else `DARK` |
| P5 Persistence round-trip | `mode ∈ {DARK,LIGHT,SYSTEM}` against an in-memory secure-store fake | `setMode(m)` then reload yields `m`; persisted blob is `{ version: PREFERENCE_VERSION, mode: m }`; no network/profile write invoked |
...
- **Resolution gate render (Req 3.3):** with `SplashScreen` mocked, the provider renders `null` (no themed child) until both `load()` resolves and the OS scheme is read, then mounts the themed tree once and calls `hideAsync` — asserting a single themed paint (no default-then-repaint).
...
- **App-wide re-render (Req 4.2, 7.2):** multiple `useTheme` consumers under one provider all observe the new `resolvedTheme` after `setMode`; no stale consumer; no restart.
...
- **Hex-guard scan (Req 1.1, 1.5, 4.3, 5.1, 5.3, 6.1, 6.2):** a CI check greps application source for raw brand hex in **color values** (`#0B0C10`, `#1F2833`, `#00F5D4`, and general `#RRGGBB` in style objects) and fails if any appear **outside** the token layer (`primitives.ts` / `*.theme.ts`). Scoped to application color values; **excludes** external SVG/raster/branding assets and third-party component internals to avoid false positives (REQ-TH1, REQ-TH9).
...
| `palette` (primitive hex) | `primitives.ts` | The single allowed home for raw brand hex, besides the theme value files (Req 6.1, 6.2) |
