...
- [x] 11. Implement PATCH /profile/me/cleaner endpoint (update cleaner-specific fields in cleaner_profiles: specialties, work_zone_center, work_zone_radius_km, work_zone_label, availability + bio in profile_details — Cleaner role guard, validate specialties array, validate availability JSONB schema, validate bio max length)
- [x] 12. Implement POST /profile/me/photo endpoint (multipart upload, validate format and size, resize, store in MinIO, update photo_storage_key in profile_details, delete old photo if exists)
- [x] 13. Implement DELETE /profile/me/photo endpoint (remove photo from MinIO, set photo_storage_key to NULL)
- [x] 14. Implement GET /profile/:userId public profile endpoint (dedicated repository method findPublicProfile that SELECTs only public columns — display_name, photo_storage_key, member_since, bio, specialties, work_zone_label, KYC badge status — NEVER selects email, phone, settings, exact coordinates)
- [x] 15. Implement portfolio service (upload photo to MinIO with user subfolder, validate max count from env, manage display_order, delete photo from MinIO + DB, derive portfolio completeness from COUNT(*))
...
- [x] 25. Implement account deletion request handler (POST /profile/me/delete-account — validate confirmation word, check no active services, mark user DELETION_PENDING, disable Keycloak account, enqueue BullMQ job, return 202)
...
- [x] 30. Implement SettingsScreen (language selector with immediate i18n reload, theme toggle with immediate apply, notification preference toggles, sync to backend)
...
- [x] 34. Create i18n translation files for profile module (en/profile.json, es/profile.json — all screen labels, error messages, settings options, confirmation dialogs)
