...
- **`posture` is a three-valued enum `{ FLAT, HALF_OPEN, UNKNOWN }`, and `UNKNOWN` is never conflated with `FLAT`.** `UNKNOWN` means the platform does not expose posture; in that case the layout decides by `sizeClass` alone. Posture-aware arrangements are an *optional* enhancement (`MAY`) applied only where the platform exposes `HALF_OPEN` and it adds value.
- **The adaptive layer causes no state loss; existing owners keep their state.** A fold/unfold/rotate is an Android **configuration change within the same running session** (explicitly *not* process death / app restart). Navigation stack, form input, in-progress actions (call / tracking / upload / timer / open sheet), scroll position, authenticated session, and theme are each **owned by their existing owner**; the adaptive layer only rearranges presentation and **never resets or destructively remounts** them.
...
5. **The adaptive layer never owns owned state, and never destructively remounts.** Adaptive containers rearrange children with **stable React keys/identity** so a configuration change relays out without remounting the navigation container, forms, active-action screens, or lists. Continuity is an invariant of "the adaptive layer only rearranges", not a new persistence mechanism (REQ-SM5).
...
│   ├── paneGeometry.ts                      (pure: (window, hinge, insets) → { listRect, detailRect } non-overlapping, hinge-safe)
...
flowchart TD
    ...
    PT -->|HALF_OPEN exposed| Opt[MAY apply posture-aware arrangement where valuable]
    PT -->|FLAT| Note
```
...
sequenceDiagram
    ...
    Adapt->>UI: re-lay-out with STABLE keys/identity (rearrange, do NOT remount)
    Adapt-->>Nav: navigation container NOT remounted — stack/route preserved
    Adapt-->>Own: owner components NOT torn down — input/action/scroll/session/theme preserved
    Note over Adapt,Own: adaptive layer's only obligation: do not CAUSE loss. Each owner keeps its own state.
...
//   posture is advisory only: HALF_OPEN MAY refine arrangement where exposed; UNKNOWN/FLAT do not change the size-class decision.
...
// paneGeometry.ts — computes non-overlapping, hinge-safe list/detail rects for a two-pane window.
export function computePaneGeometry(input: {
  window: { width: number; height: number };
  insets: SafeAreaInsets;
  hinge: Rect | null;
}): { listRect: Rect; detailRect: Rect };
// Guarantees: both rects lie within the safe window bounds, are pairwise non-overlapping,
// and neither intersects the hinge occlusion region when a hinge is present.
```
...
- `HALF_OPEN` (where the platform exposes it) MAY drive a posture-aware arrangement (e.g. content in the upper half, controls in the lower half) on flows where it adds value; where posture is `UNKNOWN` the layout falls back to the `sizeClass` decision **without error**.
...
- In a two-pane map/radar layout, `computePaneGeometry` guarantees the map pane is not clipped by the fold or the detail pane and remains fully interactive in its own rect.

### Galaxy Store publish readiness (REQ-SM8 — build/docs, not code)

- **Single Expo/RN codebase → Android AAB.** No Samsung-brand branching in app logic (structurally guaranteed by the brandless resolver + a CI brand-guard scan). The same build config that produces the Play Store AAB produces the Galaxy Store AAB.
...
### Property 6: State continuity — a configuration change preserves all owned state and never destructively remounts

*For all* owned-state snapshots (navigation stack + route, form input, in-progress action state such as an active call/tracking/upload/timer/open sheet, list scroll offset, authenticated session, and active theme) and *for all* finite sequences of configuration-change events (fold / unfold / rotate) applied within the same running session, the adaptive re-layout SHALL be an **identity over owned state**: each owner's state after the sequence SHALL equal its state before, the navigation container and owner components SHALL NOT be remounted or torn down by the adaptive layer, and no event SHALL cause a crash or a destructive whole-tree remount. (Scope: within-session configuration changes only — process death / app restart is explicitly out of scope.)
...
### Property 8: Two-pane geometry is within bounds, non-overlapping, and hinge-safe across the supported universe

*For all* window sizes and orientations in the supported universe (`{COMPACT, MEDIUM, EXPANDED} × {PORTRAIT, LANDSCAPE}`), with or without a hinge/cutout, `computePaneGeometry` SHALL produce a `listRect` and `detailRect` that both lie entirely within the safe window bounds (respecting `safeAreaInsets`), are pairwise **non-overlapping**, each meet a minimum usable size, and — when a hinge `Rect` is present — neither pane's interactive content region intersects the hinge occlusion region. No configuration in the supported universe SHALL yield clipped, overlapping, or unusable pane geometry.
...
| Condition | Behavior |
|-----------|----------|
| Platform does not expose foldable posture | `derivePosture` → `Posture.UNKNOWN`; layout decides by `sizeClass` alone; never conflated with `FLAT`; no error (Req 1.2, 4.1) |
...
| Configuration change (fold/unfold/rotate) mid-session | Adaptive layer re-lays out with stable keys; owners keep state; no remount, no crash, no lost session/theme (Req 3.1–3.5) |
...
| Posture-aware arrangement requested where posture unavailable | Silently fall back to the size-class arrangement (the `MAY` is skipped); no error (Req 4.1) |
...
| P2 Layout decision | `sizeClass × posture × isPrioritized` (all combinations, generated) | `TWO_PANE` iff `EXPANDED ∧ prioritized`; `REFLOW` for MEDIUM / EXPANDED-non-prioritized; `SINGLE_PANE` for COMPACT; `UNKNOWN` result == size-class-only result (never FLAT branch); equal `(sizeClass, isPrioritized)` → equal decision |
...
| P6 State continuity | Arbitrary owned-state snapshots (nav/form/action/scroll/session/theme) + arbitrary config-change sequences | State after == before (identity); navigation container + owners not remounted/torn down; no crash; no destructive root remount |
...
| P8 Pane geometry | Window sizes × orientations over the supported universe, with/without hinge + insets | `listRect`/`detailRect` within safe bounds, non-overlapping, each ≥ min usable size, neither intersects hinge occlusion when present |
...
- **State continuity (Req 3.1–3.5):** render a tree with a mounted form + a stubbed active-action owner + a scrollable list under the adaptive layer; simulate a dimensions/posture change; assert form value, action-owner state, scroll offset, session, and theme are unchanged and no owner `unmount`/teardown fired.
- **Posture fallback (Req 4.1):** `derivePosture(null) === UNKNOWN`; a screen with `posture = UNKNOWN` renders the size-class layout without error; `HALF_OPEN` on a supporting flow renders the posture-aware arrangement (example).
...
- **Pane geometry non-overlap + hinge (Req 4.4):** for a representative unfolded window with a hinge rect, assert map and detail rects are disjoint and clear of the hinge.
...
- **Single-codebase AAB:** the mobile build produces an Android AAB from the same Expo codebase with no per-store code branch; store identifiers come from build config.
...
- **Reads (does not own)** each prioritized flow's selection id from navigation params / shared store, and reads — but never owns or resets — navigation, form, in-progress action, scroll, session, and theme state (REQ-SM5, REQ-SM5b).
