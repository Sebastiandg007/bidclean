...
- [x] 20. Implement face comparison endpoint in FastAPI (POST /ai/face-compare — DeepFace similarity calculation between selfie and document face)
...
- [x] 24. Implement Selfie Capture Screen (front camera, face-shaped overlay, single face validation, capture guidance with i18n)
- [x] 25. Implement KYC Status Screen (status display per state, retry button for REJECTED, banner/CTA for incomplete, progress indicator for PROCESSING)
