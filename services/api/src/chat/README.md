...
| `listeners/offer-terminal-chat.listener.ts` | Decoupled `@OnEvent` listener that closes an offer's conversations when it becomes terminal (CANCELLED/EXPIRED/COMPLETED → `ChatService.closeConversationsForOffer`, P17). MATCHED is observed but a no-op by design (a match is when chat becomes usable). Closing is idempotent and best-effort — failures are logged, never propagated. Mirrors negotiation's `OfferTerminalListener` rather than a direct call from offers. |
| `chat.module.ts` | Wires the module: registers `ChatController` + providers (including the `OfferTerminalChatListener`), binds the `CHAT_REALTIME_PUBLISHER` seam to the existing `CentrifugoClient` (`useExisting`, imported via `OffersModule`) so the service depends on the interface not the HTTP client, and exports `ChatParticipationService` (for the auth Centrifugo endpoint) and `ChatService`. Runs `validateChatConfig()` on init (fail-fast). |
...
| `__tests__/offer-terminal-chat.listener.spec.ts` | `OfferTerminalChatListener` with a mocked `ChatService`: CANCELLED/EXPIRED/COMPLETED each close the offer's conversations idempotently (P17), MATCHED never closes (chat opens at match), and a close failure is swallowed (best-effort, never propagates). |
| `__tests__/chat-properties.spec.ts` | Property-based tests (fast-check, ≥100 runs): idempotent send is payload-checked and persists exactly one message across arbitrary retries, changed body → conflict (P5); sequential sends get unique, strictly increasing sequence numbers (P6); `before`/`after` keyset paging over a random history has no gaps/overlaps and reconstructs the full log exactly once (P8); connection-token subject is always the authenticated caller and a subscription token is issued only to a participant by lookup, never the channel string (P10). |
| `__tests__/chat-scenarios.spec.ts` | Scenario/integration suite complementing `chat.integration.spec.ts`: authorization & lifecycle — unmatched thread has no conversation, non-participant denied read/write, offer-terminal match invalidation closes the conversation and blocks new sends while history stays readable (P3/P17); transport resilience — a publish failure still persists (no message lost) and a reconnecting client reconciles missed messages via the `after` cursor with no duplicates and correct order (P4/P9). |
| `__tests__/chat-deletion-coherence.spec.ts` | Account-deletion coherence (P18) asserted at the migration DDL: participant FKs (`host_id`, `cleaner_id`) and `sender_id` are `ON DELETE SET NULL` — never `CASCADE` from `users` — so anonymizing/deleting a participant can never destroy a shared conversation or its history; only `thread_id`/`offer_id` cascade. Guards against a future migration edit reintroducing a user-cascade (Requirements 8.2, 8.3). |
