...
### Configuration (RevenueCat dashboard — no repo code change)
- **RevenueCat offerings, packages, entitlement mapping & AI paywalls configured (Spec — revenuecat-subscriptions)** — configured the RevenueCat project (`projd5e05a7f`, Test Store app) via the RevenueCat MCP to match what the mobile code already expects. Created the two role offerings `cleaner_pro` and `host_pro` (the code's `RC_OFFERING_IDS`), each with Monthly / Yearly / Lifetime packages (`$rc_monthly`/`$rc_annual`/`$rc_lifetime`) attached to the shared `monthly`/`yearly`/`lifetime` products; mapped all three products to the `ad_free`, `cleaner_pro`, and `host_pro` entitlements (any PRO plan unlocks the role PRO tier plus ad-free); set `cleaner_pro` as the current offering. Generated two dark-themed AI paywalls (BidClean branding: `#0B0C10` background, `#00F5D4` accent CTA, Yearly highlighted) as **unpublished drafts** — not published, not attached, pending operator review in the RevenueCat editor. Product prices already existed and were left untouched. No secret values touched: the MCP is authenticated separately; the `sk_` secret key remains a `SECRET_EXPOSURE` finding to be rotated.

### Technical debt (owed before store launch)
- **RevenueCat per-role pricing not yet split (ADR-011)** — the Test Store uses shared `monthly`/`yearly`/`lifetime` products with a single price per plan, so during sandbox/demo both Cleaner PRO and Host PRO show the same per-plan price. The business model wants Cleaner PRO 4.99/mo vs Host PRO 9.99/mo. Real prices are defined in the store console (Play Store / App Store), not RevenueCat, and only the sandbox Test Store is configured now. **Remediation (pre-launch):** when the real store app configuration is added, split products per role (`cleaner_*` vs `host_*`) with correct prices, re-attach to the role offering packages and to the role PRO + `ad_free` entitlements, and remove the shared placeholders. Decision recorded in **ADR-011 (RevenueCat shared products with single per-plan pricing — test-store phase)**.

### Added
- **Secrets Inventory — complete tooling, reconciled `.env.example`, tests & ADR-010 (Spec — secrets-inventory)** — completed the `secrets-inventory` meta-spec end to end. Added the six per-source scanners (`sources/application-scanner.ts` covering `*.constants.ts` + `validateXxxConfig()` keys + NestJS `ConfigService.get/getOrThrow` reads + the `ENTITLEMENT_ID_MAP` indirection + AI pydantic `BaseSettings` + mobile `EXPO_PUBLIC_*`, plus `build-`, `infra-`, `ci-`, `deploy-`, and `runtime-scanner.ts`) behind `sources/index.ts`, the `merge.ts` model layer (merge by name, union provenance), `classify.ts` (kind/`requiredScope`/`envApplicability`/placeholder + `checkBoundary`), `reconcile.ts`, `report.ts`, `inventory.ts` (pure orchestration), `inventory.cli.ts` (the `config-inventory` CI entry point, non-zero exit on any blocking finding), and `orphan-justifications.ts` (structured, accountable, time-bounded `OrphanJustification`s for genuinely-declared-but-unwired vars). Reconciled the committed `.env.example` against the canonical model: appended the 62 variables a source reads but the file lacked (mobile `EXPO_PUBLIC_*` tunables, `KEYCLOAK_BASE_URL`, `POSTGRES_DB/USER`, `CENTRIFUGO_TIMEOUT_MS`, `ONESIGNAL_API_URL/TIMEOUT_MS`, `PROFILE_DELETION_BACKOFF_DELAY_MS`, `SNAPSHOT_RATE_LIMIT_MS`) and annotated validator-required entries so documented required/optional matches the validators. Integration run over the real tree: **196 variables, all with provenance, zero `MISSING_IN_ENV_EXAMPLE`, zero unjustified `ORPHANED_ENV_EXAMPLE`, zero `REQUIRED_MISMATCH`, zero `SECRET_ON_CLIENT`, AI surface holds no storage credential**, and the tracked `.kiro/settings/mcp.json` RevenueCat key is reported as the single blocking `SECRET_EXPOSURE` (untouched → repo not compliant), exactly as designed. Added the 12 property-based suites (fast-check, ≥100 iterations, each tagged `// Feature: secrets-inventory, Property {n}`) mapping one-to-one to Properties 1–12, plus example-based (parser/classifier/`requiredScope`) and real-tree integration suites; `tools/config-inventory/{tsconfig.json,jest.config.js}` wire ts-jest against the hoisted workspace deps. Recorded **ADR-010 (configuration inventory + source taxonomy + orthogonal `requiredScope`/`envApplicability` axes + public/secret boundary + no-rotation-this-iteration)** and refreshed `docs/CONFIGURATION-INVENTORY.md` (generated projection) + `tools/config-inventory/README.md`. Verification: `tsc --noEmit` clean, `eslint --max-warnings 0` clean, `jest` 25/25 green. No secret values are ever emitted (placeholders only) and nothing is rotated, moved, or staged.
- **Secrets Inventory — exposure & hygiene scanner (Spec — secrets-inventory)** — added `tools/config-inventory/exposure-scanner.ts`, the real secret-exposure verifier for the `secrets-inventory` tooling. It runs `git check-ignore` on the runtime env files (`.env`, `.env.local`, `.env.staging`, `.env.production`), a tracked-file scan via `git ls-files` (a file can be in `.gitignore` yet already tracked), and a secret-pattern scan over tracked files using generic + provider-specific detectors (Stripe, AWS, RevenueCat, OneSignal, LiveKit, private-key blocks), skipping `.env.example` placeholders by design. Every hit becomes a blocking `SECRET_EXPOSURE` `Finding` that references the file, line, and matched provider/pattern — never the captured value — and the scanner NEVER mutates, moves, or rotates anything. Framing is honest: a clean run means only "no KNOWN secret-pattern exposure detected", never proof of absence; `git` being unavailable raises `GitUnavailableError` and is treated as blocking/inconclusive, never a false "clean". The `GitRunner` seam (`isIgnored`/`listTrackedFiles`) is injectable so the suite can drive it without a real repo. Updated `tools/config-inventory/README.md` to move `exposure-scanner.ts` and `reconcile.ts` from Planned into the implemented Files table (and documented the previously-undocumented `merge.ts`). No `docs/ARCHITECTURE.md` change required — §5g already documents this tooling (ADR-010) and its "reconcile + classify + exposure scan" node covers the exposure scan; this is a file within already-diagrammed tooling, below diagram granularity. No new steering file or hook warranted (follows existing `clean-code` + strict-TypeScript conventions; enforcement is the spec-owned `config-inventory` CI job, not a Kiro IDE hook). Nothing hardcoded — detectors are named constants, no real secret values.
- **Secrets Inventory — canonical inventory model (Spec — secrets-inventory)** — added `tools/config-inventory/inventory.model.ts`, the first file of the new `secrets-inventory` tooling and the single derived source of truth for every external configuration input's existence, classification, and requiredness. It defines the canonical types: `ConfigVariable` (with `surface`, `group`, `kind`, `requiredScope`, `envApplicability`, `placeholder`, `consumedBy`, `provenance`), the `SourceType` taxonomy (`APPLICATION | BUILD | DEPLOY | INFRA | CI | RUNTIME`) that completeness is defined against, `DiscoveryProvenance` (where a variable was discovered), the two orthogonal axes `RequiredScope` (`runtime | build | deploy | infra`) and `EnvApplicability` (`local | staging | production`) kept deliberately non-conflated, the structured `OrphanJustification` (never free text), the `Finding`/`FindingCode`/`InventoryReport` reconciliation-output contracts, and the per-scanner `DeclaredVariable`/`SourceScanner` seam. The model encodes the authority chain `CODE / CONFIG SOURCES → CANONICAL MODEL → .env.example → RUNTIME ENV` — `.env.example` is a generated PRESENTATION projection, never an authority. Created the new-folder `tools/config-inventory/README.md` (the parent had none, and `tools/` is a new top-level tooling location) documenting purpose, the implemented-vs-planned file set per `secrets-inventory/design.md`, dependencies, and the no-rotation / ends-at-"configured & startup-valid" scope boundaries. Added **§5g "Configuration Surfaces & Public/Secret Boundary"** to `docs/ARCHITECTURE.md` with a Mermaid diagram of the six config-source types → canonical model → API/AI/MOBILE/INFRA surfaces and the hard public/secret boundary (blocking `SECRET_ON_CLIENT` / `SECRET_EXPOSURE`), and bumped the doc's last-updated date. Types-only file — no runtime behavior, no database entities, nothing hardcoded. No new steering file or hook warranted: the tooling follows existing `clean-code` + strict-TypeScript conventions already covered by steering, and its drift/exposure enforcement is a spec-owned CI job (the planned `config-inventory` job), not a Kiro IDE hook. The configuration-inventory + source-taxonomy + orthogonal-axes + public/secret-boundary + no-rotation decisions are recorded in the spec as the planned ADR-010.
- **Realtime Chat spec complete (Spec 13 — realtime-chat)** — finished the full-stack post-match Host↔Cleaner messaging feature (all obligatory and optional `*` tasks) and closed the design audit. PostgreSQL is the source of truth; Centrifugo is transport only (persist-then-publish, publish best-effort — a transport failure never loses a message, recovery is via the `after` reconciliation cursor). Completed the mobile side on top of the already-landed backend: the `useChatStore` Zustand store (optimistic send keyed by `clientMessageId` with timeout→`failed`, upsert/dedup by `id`+`clientMessageId` in `sequenceNumber` order, keyset `before`/`after` paging), the `useChatChannel` raw-WebSocket lifecycle (connection + per-channel subscription tokens, bounded backoff, reconcile-on-(re)connect), `ChatScreen` + `ChatEntryScreen` + `MessageBubble`/`MessageComposer`/`ConversationHeader`, en/es i18n at full parity, and navigation from both roles (Host Offers stack from a matched offer, Cleaner Active-tab stack). Added property-based backend suites (fast-check, ≥100 runs) for idempotent payload-checked send (P5), strictly-increasing sequence order (P6), keyset history both cursors (P8), and Centrifugo token scoping by authenticated subject + participation lookup (P10), plus scenario/integration suites for authorization+lifecycle (P3/P17) and transport resilience (P4/P9/P19); the mobile store carries the P13 client-dedup property. **Audit fixes:** corrected the `ChatScreen` message list to an inverted (newest-at-bottom) FlatList so backward-scroll pagination (`onEndReached` → `loadOlder`) triggers at the older end (P8), and clarified the inbox-summary refresh path in `onIncomingMessage`. No secrets or business-rule values hardcoded (all env/constants); i18n keys authoritative; authorization always by JWT subject, never client identity or channel string; message bodies never logged verbatim (P15). Verification: `services/api` `tsc --noEmit` + ESLint (`--max-warnings 0`) clean and full jest green (136 suites / 1442 tests); `apps/mobile` `tsc --noEmit` clean, chat/nav ESLint clean, full jest green (51 suites / 474 tests). Recorded as **ADR-009 (PostgreSQL as chat source of truth, Centrifugo as best-effort transport; auth-owned token issuance)**. Marked Spec 13 ✅ in `.kiro/specs/ROADMAP.md` (Sprint 4 now current).
- **Chat — Cleaner Active-tab stack wired to chat entry (Spec — realtime-chat)** — added a lightweight local `ActiveStackNavigator` to `apps/mobile/src/navigation/CleanerNavigator.tsx` so the Cleaner "Active" tab now owns a stack (active-jobs list placeholder → `ChatEntryScreen`), mirroring the Host Offers stack pattern (`OffersStackNavigator`): it tracks a `StackEntry[]` with a `useMemo`-stable `navigate`/`goBack` seam and routes to `ChatEntryScreen` on the shared `CHAT_ROUTE` constant, so a Cleaner can open the conversation for a matched thread from their active jobs. Added the accent chat-button styles (`chatButton`/`chatButtonPressed`/`chatButtonText`) to `apps/mobile/src/screens/offers/OfferDetailScreen.tsx` for the offer-detail entry point into chat. All copy stays i18n-driven and all colors/sizes remain local design-token constants (`#0B0C10` background, `#00F5D4` accent) — nothing hardcoded in logic. No `docs/ARCHITECTURE.md` change required — the frontend diagram already contains the `Chat Screen` node and this is mobile-only navigation wiring below diagram granularity, following the established local-stack-navigator + injected-`navigation`-seam convention (no new steering file or hook warranted).
- **Chat — ChatEntryScreen navigation container (Spec — realtime-chat)** — added `apps/mobile/src/screens/chat/ChatEntryScreen.tsx`, the navigation container that resolves a matched `threadId` into a conversation before rendering the chat: it calls the store's idempotent open-or-get, shows an accent `ActivityIndicator` while resolving and an i18n error state on failure (`testID`s `chat-entry-loading` / `chat-entry-error`), then mounts `ChatScreen` with the resolved `conversationId`. This keeps the resolve step out of `ChatScreen` so it stays focused on a known conversation. Wired into the Host Offers stack via a new `CHAT_ROUTE` constant in `chat.constants.ts` (routed in `HostNavigator.tsx`); all copy from i18n and all colors/sizes are local design-token constants (`#0B0C10` background, `#00F5D4` accent). Documented the new file in the `apps/mobile/src/screens/chat/README.md` Files table. No `docs/ARCHITECTURE.md` change required — the frontend diagram already contains the `Chat Screen` and `useChatStore` nodes; this is a mobile-only screen container below diagram granularity, following the established feature-folder + i18n conventions (no new steering file or hook warranted).
- **Chat — ChatScreen + ConversationHeader with unit tests (Spec — realtime-chat)** — added `apps/mobile/src/screens/chat/ChatScreen.tsx` (the conversation view for a matched Host↔Cleaner pair) and `apps/mobile/src/screens/chat/components/ConversationHeader.tsx` (top bar with back affordance, title, and a connection-status indicator). `ChatScreen` composes the `useChatStore` (single source of message state + optimistic send) with `useChatChannel` (incoming messages + reconcile-on-(re)connect) and the presentational components (header, message list, composer): it loads the latest history page on mount, decides own vs counterparty by the authenticated user id (local optimistic sends with a `sendState` are always own), renders the empty state and — when the conversation is `CLOSED` — hides the composer behind a closed notice, and surfaces store errors through an i18n banner. All copy comes from i18n and all colors/sizes are local design-token constants (`#0B0C10` background, `#00F5D4` accent) — nothing hardcoded in logic. Added `apps/mobile/src/screens/chat/__tests__/ChatScreen.spec.tsx` (own vs counterparty rendering, failed send-state affordance, closed-conversation composer hiding, empty state; `useChatChannel`/`chat.api` mocked) and `__tests__/useChatChannel.spec.ts`, and extended `__tests__/chat.store.spec.ts`; moved `ChatScreen.tsx` and `components/ConversationHeader.tsx` from planned to implemented in `apps/mobile/src/screens/chat/README.md` (with the three test files listed). No `docs/ARCHITECTURE.md` change required — the frontend diagram already contains the `Chat Screen` and `useChatStore` nodes; these are mobile-only screen/component files below diagram granularity, following the established feature-folder + Zustand-selector + i18n conventions (no new steering file or hook warranted).
- **Chat — mobile WebSocket channel hook (Spec — realtime-chat)** — added `apps/mobile/src/screens/chat/useChatChannel.ts`, the client transport hook for realtime delivery on a single conversation. It mirrors the radar's `useCentrifugoChannel` lifecycle (raw WebSocket, no `centrifuge-js`): fetches a connection token + a per-channel subscription token from the auth-owned endpoint, connects to `chat:conversation:{id}`, unwraps the Centrifugo push envelope (`data.result.data` or `data.push.pub.data`), reconnects with bounded exponential backoff (1s → 2s → … → 30s), and on every (re)connect calls back so the caller reconciles missed messages via the store's `after` cursor (P19) — so recovery never depends on immediate realtime delivery. It subscribes on mount and tears down on unmount (no duplicate subscriptions), validates + narrows each frame into a `ChatMessage` (malformed frames ignored), and is transport-only: message merge/dedup/order stay in `chat.store.ts`, and it never persists or dedups. Moved `useChatChannel.ts` from planned to implemented in `apps/mobile/src/screens/chat/README.md`. No `docs/ARCHITECTURE.md` change required — the frontend diagram already contains the `Chat Screen` and `useChatStore` nodes and the backend `Centrifugo` node with its `Chat --> Centrifugo` edge; this is a mobile-only transport hook below diagram granularity. Introduces no new pattern warranting a steering file or hook — it follows the established `useCentrifugoChannel` raw-WebSocket + bounded-backoff + reconcile-on-reconnect convention already used by the offer radar.
- **Chat — mobile domain types + module README (Spec — realtime-chat)** — added `apps/mobile/src/screens/chat/chat.types.ts`, the first file of the mobile `chat` feature: the pure, SDK-free domain contracts mirroring the backend chat views (`ChatConversation`, `ChatConversationSummary`, `ChatMessage`, `ChatSendResult`, `ChatMessagePage`, `ChatMessageEvent`) plus the local-only `SendState` (`sending`\|`sent`\|`failed`) for optimistic UI and the `ConnectionStatus` union surfaced to the WebSocket lifecycle. Messages carry the server-authoritative `sequenceNumber` (ordering) and `clientMessageId` (own-send dedup), matching the backend's total-order + idempotent-send invariants (P6/P5/P13); `sendState` is explicitly never persisted server-side. Created `apps/mobile/src/screens/chat/README.md` — the one missing module README on the mobile side (every sibling screen module already had one) — documenting purpose, the implemented vs planned file set (per `realtime-chat/design.md`), dependencies, the REST + Centrifugo-token endpoints consumed, `EXPO_PUBLIC_CHAT_*` env vars, and the message ordering/dedup model. No `docs/ARCHITECTURE.md` change required — the frontend diagram already contains the `Chat Screen` and `useChatStore` nodes and the backend `Chat Module` node with its edges; this is a mobile-only types file below diagram granularity. Introduces no new pattern warranting a steering file or hook — it follows the established feature-folder + typed-contract + Zustand-store convention used across the other screen modules.
- **Chat — offer-terminal conversation close listener (Spec — realtime-chat)** — added `services/api/src/chat/listeners/offer-terminal-chat.listener.ts`, a decoupled `@OnEvent` listener that closes an offer's chat conversations when the offer becomes terminal (CANCELLED / EXPIRED / COMPLETED → `ChatService.closeConversationsForOffer`, P17); MATCHED is observed but intentionally a no-op since a match is exactly when chat becomes usable. Closing is idempotent and best-effort — a failure is logged (never the message body) and never propagated, with the negotiation reconciliation sweep and these same events providing self-healing. The listener is wired as a provider in `chat.module.ts` and mirrors negotiation's `OfferTerminalListener` rather than a direct call from offers (offers stays unaware of chat). Added `services/api/src/chat/__tests__/offer-terminal-chat.listener.spec.ts` (CANCELLED/EXPIRED/COMPLETED each close idempotently, MATCHED never closes, close failure swallowed) and updated `services/api/src/chat/README.md` (Files + Testing tables, module wiring note). No `docs/ARCHITECTURE.md` change required — the backend `Chat Module` node and its `Events --> Chat` / `Chat --> DB` edges already capture this flow; this is an event-listener below diagram granularity following the established decoupled `@OnEvent` convention (no new steering file or hook warranted).
- **Chat — module wiring + realtime-publisher DI fix (Spec — realtime-chat)** — added `services/api/src/chat/chat.module.ts` and registered `ChatModule` in `app.module.ts`, completing the backend `chat` module. The module binds the `CHAT_REALTIME_PUBLISHER` seam to the existing `CentrifugoClient` via `useExisting` (imported through `OffersModule`) so `ChatService` depends on the `ChatRealtimePublisher` interface rather than the HTTP client, exports `ChatParticipationService` + `ChatService`, and runs `validateChatConfig()` on init (fail-fast). `auth.module.ts` now imports `ChatModule` and registers the previously-added `CentrifugoController` + `CentrifugoTokenService`, closing the auth↔chat boundary (auth issues Centrifugo tokens, chat owns the participation rule). **Fixed a runtime DI defect** in `chat.service.ts`: the constructor injected `ChatRealtimePublisher` (a TypeScript interface, absent at runtime) without a token, so Nest could not resolve it and the module would fail to boot — added `@Inject(CHAT_REALTIME_PUBLISHER)` on the parameter so the `useExisting` binding resolves. Also mapped `created_at`/`updated_at` in the in-memory chat test data source so integration specs project those columns. Added `services/api/src/chat/__tests__/chat.integration.spec.ts` (ChatService over the real ChatRepository + in-memory DataSource: match → open → send → history end-to-end). Full chat suite green (3 suites / 29 tests); `services/api` `tsc --noEmit` and ESLint clean. No `docs/ARCHITECTURE.md` change required — the backend `Chat Module` node and its edges already exist; this is module composition below diagram granularity, following the established Nest module + injected-seam conventions (no new steering file or hook warranted).
- **Chat — REST controller for post-match messaging (Spec — realtime-chat)** — added `services/api/src/chat/chat.controller.ts`, the JWT-guarded `/chat` REST surface over `ChatService`: `POST /chat/threads/:threadId/conversation` (open-or-get, 200), `GET /chat/conversations` (inbox), `GET /chat/conversations/:id`, `GET /chat/conversations/:id/messages` (keyset history — `after` for reconnect reconciliation else `before`/latest, page size parsed and bounded by `CHAT_HISTORY_PAGE_SIZE`), and `POST /chat/conversations/:id/messages` (send, 201). Every handler resolves the authenticated Keycloak subject to a BidClean user server-side and passes that id to the service, so participant authorization is never based on client-supplied identity (P3); send requires both an `Idempotency-Key` header and a `clientMessageId` for idempotent retry (P5) and validates the body via a whitelisting `ValidationPipe` (P7). Moved `chat.controller.ts` + `dto/send-message.dto.ts` from planned to implemented in `services/api/src/chat/README.md` and promoted the API table from planned to live. No `docs/ARCHITECTURE.md` change required — the backend `Chat Module` node already exists and this is the HTTP surface below diagram granularity, mirroring the JWT-guarded-controller convention used across the other API modules (no new steering file or hook warranted). Verified: `services/api` `tsc --noEmit` clean and ESLint clean on the controller.
- **Chat — ChatService orchestration + per-thread match gate (Spec — realtime-chat)** — added `services/api/src/chat/chat.service.ts`, the chat domain orchestrator, and the `NegotiationRepository.isThreadMatched(threadId)` query it depends on. `openConversation` now gates conversation creation on a match for *exactly that thread* (an `ACCEPTED` proposal belonging to the thread, P1/P2) — `isThreadMatched` checks the proposal against the specific `thread_id` rather than merely the thread's offer, so a thread that never won cannot open a conversation off a sibling thread's match; it also asserts the caller is a participant (host or cleaner, P3). Reads (`getConversation`, `listConversations`, `getMessagesBefore`/`getMessagesAfter`) require participation; `sendMessage` validates + trims the body (empty → 400, over `CHAT_MESSAGE_MAX_LENGTH` → 400, P7), delegates to the repository's serialized transaction (durable before realtime, P4/P5/P6/P16/P17), then publishes best-effort to Centrifugo via the injected `ChatRealtimePublisher` seam (bound to `CentrifugoClient` in the module) — a transport failure is logged (never the body verbatim, P7) and never fails the send nor loses the message, since recipients reconcile via the `after` cursor. `closeConversationForThread` is idempotent (P1). Added `services/api/src/chat/__tests__/chat.service.spec.ts` (12 tests, repositories + publisher mocked) covering the open-time match/participant gates and the send validate→persist→best-effort-publish path, and moved `chat.service.ts` from planned to implemented in `services/api/src/chat/README.md` (with a new Testing section). No `docs/ARCHITECTURE.md` change required — the backend `Chat Module` node and its `Chat --> Centrifugo` / `Events --> Chat` edges already exist; this is service-orchestration detail below diagram granularity, following the established injected-seam + parameterized-repository conventions (no new steering file or hook warranted).
- **Chat — auth-owned Centrifugo token endpoint + participation seam (Spec — realtime-chat)** — added the token-issuance side of realtime chat, split cleanly across the module boundary (auth owns identity + token signing, chat owns the participation rule): `services/api/src/auth/centrifugo/centrifugo-token.service.ts` signs Centrifugo connection and per-conversation subscription tokens (HMAC-SHA256, bounded expiry) for the caller's own resolved user id, and `services/api/src/auth/centrifugo/centrifugo.controller.ts` exposes `GET /auth/centrifugo/token` (JWT-guarded) — with no `channel` it mints a connection token; with `?channel=chat:conversation:{id}` it mints a subscription token **only** after `ChatParticipationService.isParticipant()` confirms the authenticated subject is a participant of that conversation (the id embedded in the channel string is never trusted as proof of access, P3). Added `services/api/src/chat/chat-participation.service.ts` as the single source of the participation rule (delegating to `ChatRepository`), consumed by both chat's own authorization and the auth endpoint. The channel-prefix check reuses the shared `CHAT_CHANNEL_PREFIX` constant rather than a hardcoded literal. Added `services/api/src/auth/centrifugo/__tests__/centrifugo-token.service.spec.ts` and updated `services/api/src/auth/README.md` (Files table, API table, Dependencies) to document the new endpoint and the auth↔chat boundary; the chat README already documented the participation service. No `docs/ARCHITECTURE.md` change required — the `Centrifugo` node and the `Chat --> Centrifugo` edge already exist; this is the token-signing detail below diagram granularity, and it introduces no new pattern warranting a steering file or hook (JWT-guarded controller + injected participation seam follow the established auth/chat conventions).
- **Chat — repository, entities, types & messages (Spec — realtime-chat)** — added the persistence and contract layer of the backend `chat` module: `services/api/src/chat/chat.repository.ts` (the single owner of all `chat_conversations` / `chat_messages` reads and writes via parameterized SQL), `entities/chat-conversation.entity.ts` + `entities/chat-message.entity.ts`, `chat.types.ts` (API view/result contracts), and `chat.messages.ts`. The repository encodes the module's correctness properties: idempotent open-or-get per matched thread (one conversation per thread, P2); a serialized send under the conversation row lock — verify `OPEN` inside the lock, dedup on `(conversation_id, client_message_id)` (identical payload returns the existing row, divergent payload → conflict, P5), allocate the next `sequence_number` from the row-locked `message_seq` counter (total order, P6), insert, and bump `last_message_at` atomically (P16), closing the check-then-act race with a concurrent close (P17); keyset history via `getMessagesBefore`/`getMessagesAfter` (P8/P9); participant-only access via `isParticipant` (P3); and an idempotent `closeConversationForThread` for match invalidation (P1). Updated `services/api/src/chat/README.md` Files table to move `chat.repository.ts` from planned to implemented. No `docs/ARCHITECTURE.md` change required — the backend `Chat Module` node and its `Chat --> Cache` / `Chat --> Centrifugo` / `Events --> Chat` edges already exist; this is persistence detail below diagram granularity. Introduces no new pattern warranting a steering file or hook — parameterized-SQL repositories with row-locked serialized writes follow the established `database-standards` conventions used across the other API modules.
- **Chat — module config constants + module README (Spec — realtime-chat)** — added `services/api/src/chat/chat.constants.ts`, the first file of the backend `chat` module: env-configurable tunables (`CHAT_CONNECTION_TOKEN_TTL_SECONDS`, `CHAT_MESSAGE_MAX_LENGTH`, `CHAT_HISTORY_PAGE_SIZE`, `CHAT_CHANNEL_PREFIX`) with sensible defaults, the `chatChannelForConversation()` channel-name helper, and `validateChatConfig()` fail-fast startup validation (skipped under `NODE_ENV=test`, consistent with the other modules). Nothing is hardcoded in logic — every value derives from an environment variable. The shared `CENTRIFUGO_TOKEN_SECRET` / `CENTRIFUGO_API_URL` / `CENTRIFUGO_API_KEY` are reused (not duplicated); the previously-declared-but-unused token secret is now consumed here to sign connection + per-conversation subscription tokens. Created `services/api/src/chat/README.md` matching the sibling-module pattern (purpose, current + planned files, dependencies, planned REST API, env vars, correctness properties), filling the one documentation gap — every other API module already had a README. No `docs/ARCHITECTURE.md` change required — the backend diagram (§3) already contains the `Chat Module` node with its `Chat --> Cache` and `Chat --> Centrifugo` edges, and the `Centrifugo` node already exists in §1; this is a module-config file below diagram granularity. Introduces no new pattern warranting a steering file or hook — env-config + fail-fast validation is the established convention across the negotiation/commission/payments modules.
- **Ads — spec complete + post-implementation audit corrections (Spec 12 — revenuecat-ads)** — completed the `revenuecat-ads` module (all obligatory and optional tasks) and applied audit fixes from a design-vs-code review: (1) `useAds.initialize` now derives the provider `AdProviderContext.environment` via `resolveAdEnvironment()` instead of a hardcoded `'production'`, so a provider reading the context sees the real build environment; (2) **Req 6.4 release-on-unmount is now implemented** — `AdMobAdProvider` renders its banner through a dedicated `AdMobBannerView` wrapper that owns the native ad view 1:1 with the slot and exposes an `onReleased` teardown seam, so unmounting a slot deterministically frees the native view; (3) removed the inert `requestedRef` guard from `useAdSlot` (dead code that misrepresented the mechanism) and documented that request-once-per-mount / release-on-unmount are enforced by the render topology (one `AdBanner` → one native view per slot mount); (4) hardened the AdMob `PaidImpression.eventId` with a monotonic sequence so two same-value impressions in the same millisecond stay distinct (dedup only ever collapses genuine retries, Req 3.5 / P14). Added `apps/mobile/src/screens/ads/__tests__/admob-provider.spec.tsx` covering the empty-unit collapse and native mount/unmount teardown. Full mobile suite green (48 suites / 453 tests); mobile `tsc --noEmit` clean; ads ESLint clean. No `docs/ARCHITECTURE.md` change required — the `RevenueCat (Subscriptions + Ads)` external node already exists and these are client-side corrections below frontend-diagram granularity. Recorded as **ADR-008 (Client-Side Ads via AdProvider Seam with RevenueCat Revenue Tracking)**.
- **Ads — consent/dedup/placement/lifecycle property suites + integration tests (Spec — revenuecat-ads)** — added seven `apps/mobile/src/screens/ads/__tests__/` suites broadening coverage beyond P1 eligibility: property-based suites (fast-check, 100+ iterations) for P6 consent correctness (`consent.property.spec.ts` — `derivePersonalizationMode` total and platform-aware, Android invariant to `unavailable` ATT), P5/P14 impression dedup (`impression-dedup.property.spec.ts` — each `eventId` reported at most once, including across a simulated relaunch), slot placement cadence (`placement.property.spec.ts` — Req 1.6, ads module only fills slots and leaves the radar's `computeAdSlotPositions` intact), and P13 lifecycle idempotency (`slot-lifecycle.property.spec.ts` — re-render/unmount sequences keep `shouldRender` stable); plus end-to-end integration tests for the real `useAdVisibility` + `ad_free` path (`integration-eligibility.spec.tsx`), consent gating personalization not eligibility (`integration-consent-gating.spec.ts`, P6), graceful degradation on no-fill / provider init failure (`integration-degradation.spec.ts`, P7), and the paid-impression flow with real `createAdRevenueTracker` dedup (`integration-impression-flow.spec.ts`, P4/P5/P14). Documented all seven in the `apps/mobile/src/screens/ads/README.md` Testing table. Tests only — no `docs/ARCHITECTURE.md` change and no new pattern (fast-check property testing + injected-seam integration are established conventions).
- **Ads — Property 1 eligibility authority tests (Spec — revenuecat-ads, task 11.1)** — added `apps/mobile/src/screens/ads/__tests__/ads-eligibility.property.spec.ts`, a property-based suite (fast-check, 200/100 iterations) proving P1: an ad slot renders **iff** `adsEnabled AND providerReady AND placementAllowed AND consentResolved`, and that PRO / role / subscription-active signals never influence the render decision, while an active `ad_free` (adsEnabled=false) never yields `shouldRender=true` regardless of provider readiness, consent, or placement (Requirements 1.2, 1.3). Added a Testing section to `apps/mobile/src/screens/ads/README.md` enumerating the module's `__tests__/` suites (matching the peer subscriptions/payments READMEs) and moved `components/AdBanner.tsx` from "planned" to the implemented file table. No `docs/ARCHITECTURE.md` change required — this adds tests only (no module, integration, or data-flow change) — and it introduces no new pattern warranting a steering file or hook: property-based testing with fast-check is already the established convention (see the subscriptions/payments property suites).
- **Ads — AdSlot renders a real ad via the `useAdSlot` seam (Spec — revenuecat-ads)** — rewired `apps/mobile/src/screens/radar/components/list/AdSlot.tsx` from a static placeholder to a real ad placement: it now drives the layered render decision through `useAdSlot(RADAR_AD_SLOT_KEY)` and renders `AdBanner` (provider seam) inside the preserved BidClean dark card chrome. When `shouldRender` is false (no eligibility / provider not ready / consent unresolved / placement not allowed) or the provider yields no fill, the slot renders nothing and the radar list stays fully functional (Req 1.2 / 1.5 / P1 / P7); ads remain Cleaner-only by placement key (Req 1.7 / P11) and the component never calls RevenueCat — paid impressions flow through the hook's `onPaidImpression` to the tracker (Req 2.6). The `testID="ad-slot"`, "Sponsored" label, and accessibility label are preserved, and the now-unused placeholder design tokens and styles were removed. Lint/typecheck clean (no diagnostics). No `docs/ARCHITECTURE.md` change required — the `RevenueCat (Subscriptions + Ads)` external node and the radar screen area already exist; this rewires an existing component to existing seams below diagram granularity.
- **Ads — per-slot lifecycle hook (Spec — revenuecat-ads)** — added `apps/mobile/src/screens/ads/useAdSlot.ts`, the hook that drives a single ad slot and computes the layered render decision `shouldRender = adsEnabled AND providerReady AND placementAllowed AND consentResolved` (P1). Eligibility (`adsEnabled`) is read only from `useAdVisibility` (`ad_free`) — never PRO/tier — and the MVP asserts the Cleaner-only invariant by allowing only the known radar slot key (`placementAllowed`, P11). The hook requests an ad at most once per mount via a ref guard so list re-renders (offers changing) never re-request, and releases on unmount; `onPaidImpression` is wired to `useAds.reportImpression` so the slot never calls RevenueCat directly. Moved `useAdSlot.ts` from "planned" to "implemented" in `apps/mobile/src/screens/ads/README.md`. No `docs/ARCHITECTURE.md` change required — the high-level `RevenueCat (Subscriptions + Ads)` external node already exists and this is a mobile-only, client-side hook below frontend-diagram granularity; it introduces no new pattern warranting a steering file or hook (it follows the existing feature-folder + Zustand-selector + injected-seam convention).
- **Ads — Zustand store + consent seam (Spec — revenuecat-ads)** — added `apps/mobile/src/screens/ads/useAds.ts`, the `useAdsStore` that owns the ad provider lifecycle, consent, and impression reporting (mirroring the `useSubscription`/`usePayments` state + actions + `reset` shape), plus `apps/mobile/src/screens/ads/consent.ts`, the guarded native seam supplying `readConsentState(platform)` (ATT on iOS + UMP) so the store runs in CI without a native build. `initialize(appUserId, platform)` is idempotent and independent of the subscriptions lifecycle — it resolves consent, derives the privacy-scoped attribution id, and initializes the factory-selected provider, failing into `providerReady=false` (no ad shown, radar unaffected) on any error or when ads are disabled. `resolveConsent()` reads consent once per session and derives the `personalizationMode` (consent shapes personalization, never eligibility — `ad_free` is owned by `useAdVisibility` and is not read here), and `reportImpression()` fire-and-forgets to the `AdRevenueTracker`. Moved `useAds.ts`, `ad-provider.factory.ts`, and the new `consent.ts` from "planned" to "implemented" in `apps/mobile/src/screens/ads/README.md`. No `docs/ARCHITECTURE.md` change required — the high-level `RevenueCat (Subscriptions + Ads)` external node already exists, and this is a mobile-only, client-side store below frontend-diagram granularity; it introduces no new pattern warranting a steering file or hook (it follows the existing feature-folder + Zustand-store + injected-seam convention).
- **Ads — concrete AdMob + Mock providers (Spec — revenuecat-ads)** — implemented the two concrete `AdProvider` classes behind the SDK-free seam: `apps/mobile/src/screens/ads/providers/admob.provider.ts` (`AdMobAdProvider`, backed by `react-native-google-mobile-ads` loaded defensively so it type-checks and imports before a native prebuild — renders a banner, forwards only paid impressions (ILRD) mapped to metadata-only `PaidImpression`, owns the native ad lifecycle, collapses the slot on no-fill/error, and imports no RevenueCat nor decides eligibility) and `apps/mobile/src/screens/ads/providers/mock.provider.ts` (`MockAdProvider`, a deterministic no-SDK placeholder exposing `MOCK_AD_VIEW_TEST_ID` and `buildSyntheticImpression` for zero-real-request rendering in CI, selected under test or `EXPO_PUBLIC_ADS_PROVIDER=mock`, never a production fallback). Both moved from "planned" to "implemented" in `apps/mobile/src/screens/ads/README.md`. No `docs/ARCHITECTURE.md` change required — they sit inside the existing `AdProvider` seam below frontend-diagram granularity and introduce no new pattern warranting a steering file or hook (they follow the existing injected-seam + feature-folder convention).
- **Ads — RevenueCat ad-revenue tracker (Spec — revenuecat-ads)** — added `apps/mobile/src/screens/ads/ad-revenue-tracker.ts`, the client-side component that forwards paid impressions (ILRD) to RevenueCat's `AdTracker`. It is kept separate from the `AdProvider`/`AdSlot` so rendering and revenue tracking are independently testable: reporting is best-effort and non-blocking (every failure is swallowed and never breaks the radar), it forwards only paid, revenue-bearing impressions attributed by ad-event metadata (never PII), and it skips gracefully when RevenueCat is not configured. Duplicate protection uses an `eventId` dedup ring (in-memory + a bounded persisted set, capacity 200) so a retry, remount, or relaunch after a paid impression does not re-report — best-effort duplicate reduction, not a remote exactly-once guarantee. Collaborators (`AdRevenueSink` + `KeyValueStore`) are injected via `createAdRevenueTracker` so the tracker is pure and testable. Moved the file from "planned" to "implemented" in `apps/mobile/src/screens/ads/README.md`. No `docs/ARCHITECTURE.md` change required — the high-level `RevenueCat (Subscriptions + Ads)` external node already exists, and this is a mobile-only, client-side file below frontend-diagram granularity; it introduces no new pattern warranting a steering file or hook (it follows the existing injected-seam + feature-folder convention).
- **Ads — domain types + module README (Spec — revenuecat-ads)** — created the mobile `apps/mobile/src/screens/ads/` feature folder with its pure domain type contract (`ads.types.ts`): `AdFormat` (BANNER|NATIVE), `PersonalizationMode` (PERSONALIZED|NON_PERSONALIZED|UNRESOLVED), the two distinct consent inputs (`TrackingAuthorizationStatus` for iOS ATT + `ConsentStatus` for UMP/GDPR) kept separate in `ConsentState`, `AdPlatform`/`AdEnvironment`, `AdProviderContext` (optional identity/consent so no provider requires identity; `attributionId` is a privacy-scoped pseudonym, never the raw UUID), `PaidImpression` (`eventId`-keyed for dedup, ad metadata only — no PII), `AdViewProps`, and the abstract `AdProvider` seam (concrete: AdMob, Mock). The types are SDK-free so the core is testable without a native build. Added `apps/mobile/src/screens/ads/README.md` documenting purpose, the (implemented vs planned) file set, dependencies (`useAdVisibility`/`ad_free` from revenuecat-subscriptions, the `AdSlot` placement owned by offer-radar, AdMob + UMP, RevenueCat `AdTracker`), the layered render decision (`adsEnabled AND providerReady AND placementAllowed AND consentResolved`), and the `EXPO_PUBLIC_ADMOB_*`/`EXPO_PUBLIC_ADS_*` env vars. No `docs/ARCHITECTURE.md` change required — the high-level `RevenueCat (Subscriptions + Ads)` external node already exists, and this is a mobile-only, client-side types file below frontend-diagram granularity (the `useAdsStore`/AdMob SDK nodes will be added when the store/provider are implemented). Introduces no new pattern warranting a steering file or hook (it follows the existing feature-folder + Zustand-store convention).
- **RevenueCat Subscriptions spec complete (Spec 11 — revenuecat-subscriptions)** — all implementation tasks (including every optional `*` task) finished and verified; the `subscriptions` module is now the source of truth for a user's FREE/PRO tier, replacing the FREE-returning stub. Recorded as **ADR-007 (RevenueCat Subscriptions as a Durable Local Mirror)**. Highlights across the module:
  - **Contract extension + client endpoint.** `SUBSCRIPTION_TIER` (owned by commission-system) was extended with role-aware `getRoleTier(userId, role)` (Host tier ← `host_pro`, Cleaner tier ← `cleaner_pro`), `getTier` kept for compatibility; `SubscriberRole` added to commission types. `RealSubscriptionTierService` implements it from the mirror, and `GET /subscriptions/me` (JWT-scoped) returns the server-authoritative view (global tier + per-role tiers + active entitlements) with async self-heal when the row is missing/stale (no synchronous RevenueCat call on the request path).
  - **Durability & correctness.** Webhook ingress commits a `RECEIVED` ledger row before the ACK and a dispatch recovery worker re-enqueues orphans (no acknowledged event lost); deltas apply behind a per-entitlement out-of-order guard; TRANSFER is atomic; reconciliation converges stale rows and discovers missing subscribers; `ad_free` never implies PRO.
  - **Test coverage.** Added property-based suites (fast-check, all P1–P18) — tier derivation, webhook authenticity/dedup/ordering, sanitizer, transfer, durability, reconciliation convergence/discovery, and configuration integrity — plus a 9-case integration/scenario suite proving webhook → mirror → commission-at-match end-to-end (including the host-only-PRO P0 case, expiration, recovery, out-of-order, transfer, missed-webhook reconciliation, and account-deletion cleanup). Full `services/api` suite green (127 suites / 1379 tests); mobile suite green (31 suites / 381 tests); `services/api` + `packages/shared` typecheck and `services/api` ESLint (`--max-warnings 0`) clean.
- **Radar — ad visibility wired to the real `ad_free` entitlement (Spec 11 — revenuecat-subscriptions)** — replaced the placeholder in `apps/mobile/src/screens/radar/hooks/useAdVisibility.ts` (which always defaulted to free-tier via a simulated async check) with a real read of the server-authoritative subscription view: the hook now subscribes to `useSubscriptionStore` and hides ad slots only when an active `AD_FREE` entitlement is present (`serverView.entitlements` contains `{ key: EntitlementKey.AD_FREE, active: true }`). `ad_free` is an independent entitlement and does not imply PRO (a user can be PRO without `ad_free`, or `ad_free` without PRO); the check stays abstracted through the entitlement key so tier names can change without touching Radar. While the server view has not loaded (`serverView === null`) it reports loading and defaults to showing ads (the safe monetization fallback). Added `__tests__/useAdVisibility.spec.ts` and extended the `react-native-purchases-ui` test mock in `src/__mocks__/setup.ts` (default export now exposes `Paywall`/`PaywallFooterContainerView`/`presentPaywall`/`presentPaywallIfNeeded`) so the paywall and ad-visibility specs render. No `docs/ARCHITECTURE.md` change required — the `RevenueCat SDK` service node and the radar screen area already exist; this rewires an existing hook to an existing store below diagram granularity.
- **Subscriptions — Paywall screen (Spec 11 — revenuecat-subscriptions)** — added `apps/mobile/src/screens/subscriptions/PaywallScreen.tsx`, the mobile screen that presents the RevenueCat server-driven paywall (Paywalls V2 via `RevenueCatUI.Paywall`) for the role-appropriate offering, resolving `RC_OFFERING_IDS[role]` (Cleaner PRO for Cleaners, Host PRO for Hosts) from `Purchases.getOfferings()` with a fallback to `offerings.current`. Loading and offering-unavailable states are rendered with `subscriptions` i18n keys and design-system colors (`#0B0C10` background); on `onPurchaseCompleted`/`onRestoreCompleted` it calls the store's `refreshServerView()` so the client converges to the server-authoritative `/subscriptions/me` mirror before dismissing, and it never grants entitlements locally. Updated `apps/mobile/src/screens/subscriptions/README.md` Files table with the new screen. No `docs/ARCHITECTURE.md` change required — the `Paywall Screen (Cleaner PRO / Host PRO)` node, the `RevenueCat SDK` service node, and the backend `Subscriptions Module` already exist in the frontend diagram; this introduces no new pattern warranting a steering file or hook.
- **Subscriptions — PRO badge component (Spec 11 — revenuecat-subscriptions)** — added `apps/mobile/src/screens/subscriptions/components/ProBadge.tsx`, the first component in the subscriptions feature's `components/` folder. It renders a small accent-colored "PRO" badge for a given `role` and is gated per role from the server-authoritative view (`serverView.roleTiers[role] === PRO`), so a user who is PRO in one role and FREE in the other only sees the badge in the PRO role's view; it renders nothing when FREE. Text comes from the `subscriptions` i18n namespace (`badge.pro`) and the color is a local constant (`#00F5D4` accent on `#0B0C10`), consistent with the design system. Updated `apps/mobile/src/screens/subscriptions/README.md` with the new file entry. No `docs/ARCHITECTURE.md` change required — the subscriptions screen area, `RevenueCat SDK` service node, and backend `Subscriptions Module` already exist; this is a presentational component below diagram granularity, and it introduces no new pattern warranting a steering file or hook.
- **Subscriptions — mobile SDK, store & scaffolding (Spec 11 — revenuecat-subscriptions)** — created the mobile `apps/mobile/src/screens/subscriptions/` feature folder and added the RevenueCat SDK (`react-native-purchases`) + Paywalls UI (`react-native-purchases-ui`) to `apps/mobile`, with both native modules mocked in `src/__mocks__/setup.ts` so unit tests run without a prebuild/EAS build. The folder ships: `subscriptions.types.ts` (client mirror of the `GET /subscriptions/me` contract — `EntitlementKey` `CLEANER_PRO`/`HOST_PRO`/`AD_FREE`, `SubscriberTier`, `SubscriberRole`, `EntitlementState`, the server-authoritative `SubscriptionView`, `PurchaseResult`), `subscriptions.constants.ts` (route + `/subscriptions/me` endpoint, platform public SDK keys, entitlement/offering ids and i18n keys — all from config, none hardcoded in UI logic), `subscriptions.api.ts` (typed `GET /subscriptions/me` via the lazy shared `apiClient`), and `useSubscription.ts` (Zustand store: configures the SDK with the internal UUID as `app_user_id` + platform public key, derives client entitlements from `customerInfo` for instant UI, exposes `purchase`/`restore`, and converges to the backend mirror via `refreshServerView` after every purchase/`customerInfo` change). The client encodes the authority split: `customerInfo` is optimistic UI only and never grants access — anything affecting money/access reads the backend mirror via `/subscriptions/me`. Added `apps/mobile/src/screens/subscriptions/README.md` documenting purpose, files, dependencies, and the server-authority rule. Added a `Paywall Screen` node to the frontend Screens subgraph in `docs/ARCHITECTURE.md` to reflect the new subscriptions screen area (the `RevenueCat SDK` service node and backend `Subscriptions Module` were already present).
